...
public class Playboard {
    ...
    public Box[] getCurrentWordBoxes() {
        ...
        if(currentWord.across){
            across = currentWord.start.across-(result.length-1);
        }
    ...
    public Position getCurrentWordStart() {
        ...
        if (isAcross()) {
            ...
            int l=boxes[row].length;
            while(col < l && col+1<boxes[row].length && boxes[row][col+1]!=null){
                col++;
            }
    ...
    public void setPencil(boolean pencil){this.pencil=pencil;}

    public boolean isPencil(){return this.pencil;}
    ...
    public Box[] getWordBoxes(int number, boolean isAcross) {
        ...
        if(isAcross){
            across = start.across-(result.length-1);
        }
    ...
    public int getWordRange(Position start, boolean across) {
        if (across) {
            ...
            while(col>0 && boxes[start.down][col-1]!=null){
                col--;
            }

            return (start.across-col +1);
        } else {
            int row = start.down;
    ...
    public int getWordRange() {
        int i= getWordRange(getCurrentWordStart(), isAcross());
        return i;
    }
    ...
    public List<Position> clearPuzzle() {
        ArrayList<Position> changes = new ArrayList<Position>();
    ...
    public static class Word {
        ...
        public boolean checkInWord(int across, int down) {
            ...
            int startPos = this.across ? start.across-length+1 : start.down;
