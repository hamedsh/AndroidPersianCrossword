...
public class PlayboardRenderer {
    ...
    public PlayboardRenderer(Playboard board) {
        ...
        currentWordHighlight.setColor(Color.parseColor("#D5DBFF"));
        currentLetterHighlight.setColor(Color.parseColor("#F6EF39"));
        currentLetterBox.setColor(Color.parseColor("#E2FFF7"));
        ...
        this.pencil.setColor(Color.parseColor("#FFC166"));
    }
    ...
    public Bitmap draw(Word reset, float scale) {
        try {
            ...
            for (int row = 0; row < boxes.length; row++) {
                ...
                //for(int col=currentWord.start.across;col>(currentWord.start.across+1)-currentWord.length;col--){ //<here>
                    if (!renderAll &&
                        reset != null &&
    ...
    private void drawBox(Canvas canvas, int x, int y, int row, int col, float scale, Box box, Word currentWord) {
        ...
        if (box == null) {
            ...
            if ((highlight.across == col) && (highlight.down == row)) {
            ...
            }else if (this.board.isShowErrors() && (box.getResponse() != ' ' && !box.isPencil()) &&
                ...
                box.setCheated(true);
                canvas.drawRect(boxRect, this.red);
            }else if(box.isPencil() && !box.isCheated()){
                    canvas.drawRect(boxRect,this.pencil);
            } else {
